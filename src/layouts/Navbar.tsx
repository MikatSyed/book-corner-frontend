...
export default function Navbar() {
  ...
  //     <label className="absolute right-0 mt-1 cursor-pointer text-xl sm:hidden" >
  ...
  //         {user.email && (
  //           <> 
  //         <li className=""><a className="text-gray-600 hover:text-blue-600" href="/readinglist">Readinglist</a></li>
  //         </>
  //         )}
  //         {user.email && (
  //           <> 
  ...
  <div className="relative mx-auto flex max-w-screen-lg flex-col py-4 sm:flex-row sm:items-center sm:justify-between">
    ...
    <div className="sm:hidden ">
      ...
      <span className="ml-5 text-3xl text-blue-600">
        Book Corner
      </span>
    </div>
    ...
    <nav
      ...
      className={`${
        isOpen ? 'block' : 'hidden'
      } sm:flex sm:flex-row sm:gap-x-8 mt-4 sm:mt-0`}
    >
      <ul className="flex flex-col gap-y-4 sm:flex-row sm:gap-x-8">
        <li>
          <Link
        ...
        {user.email && (
          <>
            <li>
              <button
                className="rounded-xl border-2 border-red-600 px-6 py-2 font-medium text-red-600 hover:bg-red-600 hover:text-white"
                onClick={() => {
                  setIsOpen(false);
                  handleLogout();
                }}
